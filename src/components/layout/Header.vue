...
import { categoryNav } from "@/content/nav";
...
<template>
  <header class="top-navbar sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur-sm">
    <div class="container flex h-16 items-center justify-between gap-4">
      ...
      <nav class="hidden items-center gap-6 md:flex" aria-label="Primary">
        <RouterLink
          ...
          :key="item.href"
          :to="item.href"
          class="text-sm font-medium text-ink transition-opacity hover:opacity-70"
        >
          {{ item.label }}
        </RouterLink>
      </nav>
